...
public class LogUtils {
     ...
     * 是否同步输出到本地日志文件
     */
    public static boolean IS_SYNS = true;
    /**
     * 打印日志保存路径
     */
     ...
     */
    private static int LOG_MAX_SIZE = 4 * 1024 * 1024;
    /**
     * 检测文件目的地址是否正确
     */
...
//					}
//				}
                File f = new File(LOG_FILE_FILE);
//                Log.e("LogUtils(434)","日志文件:"+ LOG_FILE_FILE+" exists:"+f.exists());
                if (!f.exists()) {
                    try {
                        f.createNewFile();
                    } catch (IOException e) {
...
//                    Log.e("LogUtils(441)","写入日志大小:"+ Tools.getByte(f.length()));
