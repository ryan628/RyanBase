...
public class Alert2DrawView extends View {
    ...
    public Alert2DrawView(Context context, int[] location, int w, int h) {
        super(context);
        this.x = location[0];
        this.y = location[1];
        this.w = w;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int[] cl = new int[]{Constants.alertRB, Constants.alertGB, Constants.alertBB};
        ...
        canvas.drawLine(1, h, w / 5, h, p);
        canvas.drawLine((int) Math.round(w / 2.3), h, (int) Math.round(w / 1.7), h, p);
        canvas.drawLine((int) Math.round(w / 1.25), h, w, h, p);
    }
}
